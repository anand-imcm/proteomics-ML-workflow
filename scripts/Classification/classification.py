...
from Step2_VAE_MLP import vae_script as vae
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=['KNN', 'RF', 'NN', 'SVM', 'XGB', 'PLSDA', 'VAE', 'LGBM', 'LR', 'MLPVAE', 'GNB'],
...
def main():
    ...
    if 'PLSDA' in models:
        allowed_for_plsda = {'none', 'umap', 'elasticnet', None}
        if fs not in allowed_for_plsda:
            print("Skipping PLSDA because the selected feature selection method is not compatible.")
            models = [m for m in models if m != 'PLSDA']

    if fs is not None:
        if 'VAE' in models:
            print("Skipping VAE because feature selection is used.")
            models = [m for m in models if m != 'VAE']
        if 'MLPVAE' in models:
            print("Skipping MLPVAE because feature selection is used.")
            models = [m for m in models if m != 'MLPVAE']

    if not models:
        print("No models to run after applying compatibility rules.")
        sys.exit(0)

    Parallel(n_jobs=-1, verbose=100)(
        delayed(run_model)(model, args.csv, prefix, fs) for model in models
    )
