...
# -----------------------------------------------------------------------------
# Global plot font sizes (unified and enlarged)
# -----------------------------------------------------------------------------
TITLE_FONTSIZE = 22
LABEL_FONTSIZE = 18
TICK_FONTSIZE = 14
...
# -----------------------------------------------------------------------------
# PLS Feature Selector (fix: binarize y, cap n_components by classes)
# -----------------------------------------------------------------------------
class PLSFeatureSelector(BaseEstimator, TransformerMixin):
    """
    PLS-based dimensionality reduction with classification labels:
    - binarize y (one-vs-rest), force 2 columns for binary
    - cap n_components by min(n_samples-1, n_features, n_classes)
    - return DataFrame with PLS_Component_* for stable downstream use
    """
    ...
    def fit(self, X, y):
        ...
        y_onehot, classes = self._binarize_y(y)
        self.classes_ = classes
        max_comp = max(1, min(X.shape[1], X.shape[0] - 1, y_onehot.shape[1]))
...
# -----------------------------------------------------------------------------
# t-SNE transformer (safe fallback for transform)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# PCA wrapper to expose explained_variance_ratio_ if needed
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# UMAP with safe caps for components and neighbors (consider both N and p)
# -----------------------------------------------------------------------------
...
def multiclass_specificity(cm: np.ndarray) -> float:
    """
    Macro-average specificity over classes in a one-vs-rest manner:
    ...
    for k in range(K):
        ...
        specs.append(spec)
    return float(np.mean(specs)) if len(specs) > 0 else 0.0


def compute_macro_roc(y_true_binarized: np.ndarray, y_score: np.ndarray):
    """
    Compute macro-average ROC by averaging per-class TPR interpolated on the union FPR grid.
    Returns (fpr_macro, tpr_macro, auc_macro, per_class_curves) where per_class_curves is dict{i: (fpr_i, tpr_i, auc_i)}.
    """
    ...
    for i in range(n_classes):
        try:
            ...
            fpr_dict[i], tpr_dict[i] = np.array([0, 1]), np.array([0, 1])
            roc_auc_dict[i] = 0.0
    ...
    return all_fpr, mean_tpr, auc_macro, (fpr_dict, tpr_dict, roc_auc_dict)


def get_pos_proba(y_pred_prob: np.ndarray) -> np.ndarray:
    """
    Robustly extract positive-class probabilities for binary classification.
    Supports shapes: (n,), (n,1), (n,2).
    """
    if y_pred_prob is None:
        raise ValueError("y_pred_prob is None")
    ypp = np.asarray(y_pred_prob)
    if ypp.ndim == 1:
        return ypp
    if ypp.shape[1] == 1:
        return ypp.ravel()
...
def xgboost_nested_cv(inp, prefix, feature_selection_method):
    ...
    for train_idx, test_idx in cv_outer.split(X, y_encoded):
        ...
        if not tsne_selected:
            ...
            def objective_inner(trial):
                ...
                if feature_selection_method != 'none':
                    if feature_selection_method == 'pca':
                        ...
                        pls_max_components = max(1, min(X_train_outer_fold.shape[0]-1, X_train_outer_fold.shape[1], len(np.unique(y_train_outer))))
                        ...
                        l1_ratio = trial.suggest_uniform('logreg_l1_ratio', 0.0, 1.0)
            ...
            if feature_selection_method != 'none':
                if feature_selection_method == 'pca':
                    ...
                    best_C = best_params_inner.get('logreg_C', 1.0)
                    best_l1_ratio = best_params_inner.get('logreg_l1_ratio', 0.5)
                    steps.append(('feature_selection', SelectFromModel(
                        LogisticRegression(
                            penalty='elasticnet',
                            solver='saga',
        ...
        # ----------------- Inner tuning (t-SNE) -----------------
            ...
            def objective_inner_tsne(trial):
                ...
                with SuppressOutput():
                    ...
                    for inner_train_idx, inner_valid_idx in cv_inner.split(X_train_outer_fold, y_train_outer):
                        ...
                        try:
                            classes = np.unique(y_train_inner)
                            cw = compute_class_weight(class_weight='balanced', classes=classes, y=y_train_inner)
                            cw_dict = dict(zip(classes, cw))
                            sw = np.array([cw_dict[label] for label in y_train_inner])

                            model.fit(X_train_inner, y_train_inner, sample_weight=sw)
                            y_pred = model.predict(X_valid_inner)
                            f1_val = f1_score(y_valid_inner, y_pred, average='weighted')
                            f1_scores.append(f1_val)
                        except (ValueError, ArpackError, NotImplementedError):
                            f1_scores.append(0.0)
                    return float(np.mean(f1_scores))
            ...
            best_model_inner = Pipeline([
                ('xgb', XGBClassifier(
                    ...
                    reg_alpha=best_params_inner.get('reg_alpha', 0.0),
                    reg_lambda=best_params_inner.get('reg_lambda', 1.0),
                    gamma=best_params_inner.get('gamma', 0.0),
                    eval_metric='mlogloss',
                    use_label_encoder=False,
                    verbosity=0,
                    random_state=1234
        ...
        # ----------------- Fit on outer train with sample_weight -----------------
        with SuppressOutput():
            try:
                ...
                best_model_inner.fit(X_train_outer_fold, y_train_outer, xgb__sample_weight=sample_weight_outer)
        ...
        try:
            y_pred_prob_outer = best_model_inner.predict_proba(X_test_outer_fold)
            y_pred_class_outer = best_model_inner.predict(X_test_outer_fold)
        except (ValueError, ArpackError, NotImplementedError) as e:
        ...
        if num_classes == 2:
            try:
                proba_pos_outer = get_pos_proba(y_pred_prob_outer)
                fpr, tpr, _ = roc_curve(y_test_outer, proba_pos_outer)
    ...
    # ----------------- Nested CV curves (F1/AUC over folds) -----------------
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{prefix}_xgboost_nested_cv_f1_auc.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    # ----------------- Full-data hyperparameter tuning -----------------
    ...
    def objective_full(trial):
        ...
        if feature_selection_method != 'none':
            if feature_selection_method == 'pca':
                ...
                pls_max_components = max(1, min(X.shape[0]-1, X.shape[1], num_classes))
                pls_n_components = trial.suggest_int('pls_n_components', 1, pls_max_components)
                steps.append(('feature_selection', PLSFeatureSelector(
                    n_components=pls_n_components,
                    max_iter=1000,
        ...
        with SuppressOutput():
            ...
            for train_idx_full, valid_idx_full in cv_outer.split(X, y_encoded):
                ...
                try:
                    ...
                    cw_dict = dict(zip(classes, cw))
                    sw = np.array([cw_dict[label] for label in y_train_full])

                    pipeline.fit(X_train_full, y_train_full, xgb__sample_weight=sw)
    ...
    if feature_selection_method != 'none':
        if feature_selection_method == 'pca':
            ...
            best_C_full = best_params_full.get('logreg_C', 1.0)
            best_l1_ratio_full = best_params_full.get('logreg_l1_ratio', 0.5)
            steps.append(('feature_selection', SelectFromModel(
                LogisticRegression(
                    penalty='elasticnet',
                    solver='saga',
    ...
    with SuppressOutput():
        try:
            if tsne_selected:
                ...
                cw_dict = dict(zip(classes, cw))
                sw_full = np.array([cw_dict[label] for label in y_encoded])
                best_model.fit(X_transformed_final, y_encoded, xgb__sample_weight=sw_full)
            else:
                classes = np.unique(y_encoded)
                cw = compute_class_weight(class_weight='balanced', classes=classes, y=y_encoded)
                cw_dict = dict(zip(classes, cw))
                sw_full = np.array([cw_dict[label] for label in y_encoded])
                best_model.fit(X, y_encoded, xgb__sample_weight=sw_full)
    ...
    if tsne_selected:
        print(f"Best parameters for XGBoost with t-SNE: {best_params_full}")
    else:
        print(f"Best parameters for XGBoost: {best_params_full}")

    # ----------------- Save transformed data and variance info if needed -----------------
    if feature_selection_method != 'none' and not tsne_selected:
        ...
        if X_transformed is not None:
            if feature_selection_method in ['pca', 'kpca', 'umap', 'pls']:
                ...
                # Note: using SelectFromModel directly
    ...
    if tsne_selected:
        plt.title('Confusion Matrix for XGBoost with t-SNE', fontsize=TITLE_FONTSIZE, fontweight='bold')
    else:
        plt.title('Confusion Matrix for XGBoost', fontsize=TITLE_FONTSIZE, fontweight='bold')
    plt.xlabel('Predicted label', fontsize=LABEL_FONTSIZE)
    plt.ylabel('True label', fontsize=LABEL_FONTSIZE)
    plt.xticks(fontsize=TICK_FONTSIZE)
    plt.yticks(fontsize=TICK_FONTSIZE)
    plt.tight_layout()
    plt.savefig(f"{prefix}_xgboost_confusion_matrix.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if num_classes == 2:
        try:
            proba_pos = get_pos_proba(y_pred_prob)
            fpr_dict[0], tpr_dict[0], _ = roc_curve(y_encoded, proba_pos)
            roc_auc_dict[0] = auc(fpr_dict[0], tpr_dict[0])
        except ValueError:
            fpr_dict[0], tpr_dict[0], roc_auc_dict[0] = np.array([0, 1]), np.array([0, 1]), 0.0
        # micro/macro not added for binary to keep original outputs
    else:
        fpr_macro, tpr_macro, auc_macro, (fpr_c, tpr_c, auc_c) = compute_macro_roc(y_binarized, y_pred_prob)
        for i in range(y_binarized.shape[1]):
            fpr_dict[i], tpr_dict[i], roc_auc_dict[i] = fpr_c[i], tpr_c[i], auc_c[i]
        try:
            fpr_micro, tpr_micro, _ = roc_curve(y_binarized.ravel(), y_pred_prob.ravel())
            auc_micro = auc(fpr_micro, tpr_micro)
        except ValueError:
            fpr_micro, tpr_micro, auc_micro = np.array([0, 1]), np.array([0, 1]), 0.0
        fpr_dict["micro"], tpr_dict["micro"], roc_auc_dict["micro"] = fpr_micro, tpr_micro, auc_micro
        fpr_dict["macro"], tpr_dict["macro"], roc_auc_dict["macro"] = fpr_macro, tpr_macro, auc_macro
    ...
    np.save(f"{prefix}_xgboost_roc_data.npy", roc_data, allow_pickle=True)
    ...
    if tsne_selected:
        plt.title('ROC Curves for XGBoost with t-SNE', fontsize=TITLE_FONTSIZE, fontweight='bold', pad=15)
    else:
        plt.title('ROC Curves for XGBoost', fontsize=TITLE_FONTSIZE, fontweight='bold', pad=15)
    plt.legend(loc="lower right", fontsize=LEGEND_FONTSIZE, title_fontsize=LEGEND_FONTSIZE)
    plt.xticks(fontsize=TICK_FONTSIZE)
    plt.yticks(fontsize=TICK_FONTSIZE)
    plt.tight_layout()
    plt.savefig(f'{prefix}_xgboost_roc_curve.png', dpi=300, bbox_inches="tight")
    plt.close()

    # ----------------- Metrics bar chart -----------------
    metrics = {'Accuracy': acc, 'F1 Score': f1, 'Sensitivity': sensitivity, 'Specificity': specificity}
    metrics_df = pd.DataFrame(list(metrics.items()), columns=['Metric', 'Value'])
    ax = metrics_df.plot(kind='bar', x='Metric', y='Value', legend=False, figsize=(8, 6))
    if tsne_selected:
        plt.title('Performance Metrics for XGBoost with t-SNE', fontsize=TITLE_FONTSIZE, fontweight='bold')
    else:
        plt.title('Performance Metrics for XGBoost', fontsize=TITLE_FONTSIZE, fontweight='bold')
    ...
    for container in ax.containers:
        ax.bar_label(container, fmt='%.2f', padding=5, fontsize=TICK_FONTSIZE)
    plt.tight_layout()
    plt.savefig(f'{prefix}_xgboost_metrics.png', dpi=300, bbox_inches="tight")
    plt.close()
    ...
    # ----------------- If t-SNE path, also save the transformed matrix + variance note -----------------
    if tsne_selected:
