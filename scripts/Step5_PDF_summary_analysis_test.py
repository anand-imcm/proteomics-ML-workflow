...
class PDFReport(FPDF):
    ...
    def find_image_file(self, model_name, image_type=None):
        ...
        if image_type:
            ...
            expected_suffix_alt = f"{model_name}.png".lower()  # In case image_type is not part of the filename
        else:
            expected_suffix = f"{model_name}.png".lower()
            expected_suffix_alt = None

        for file in os.listdir('.'):
            if file.lower().endswith(expected_suffix):
                return file
            if expected_suffix_alt and file.lower().endswith(expected_suffix_alt):
                return file
    ...
    def add_image_with_caption(self, model_name, image_type, title, caption):
        ...
        if img_path:
            ...
            try:
                ...
                # If image loading fails, log the error and skip
                logging.warning(f"Failed to load image: {img_path}")
        else:
            # Log that the image was not found
            if image_type:
                logging.warning(f"Image not found for model '{model_name}' with type '{image_type}'. Expected suffix: '{model_name}_{image_type}.png'")
            else:
                logging.warning(f"Image not found for model '{model_name}'. Expected suffix: '{model_name}.png'")
...
pdf.add_image_with_caption(
    "output_prefix_overall_roc_curves",
    None,
    "Overall ROC Curves for All Classification Models",
    "This plot shows the ROC curves for all the classification models, providing a comparative view of their performance."
)
...
pdf.add_image_with_caption(
    "Results_reg_model_performance_metrics",
    None,
    "Regression Model Performance Metrics",
    "This table compares the evaluation metrics (MSE, MAE, RMSE, R2) for all regression models."
)
...
# Define all models (classification and regression)
all_models = list(classification_models.keys()) + list(regression_models.keys())

# Define biological image types and their descriptions
...
# Define titles for biological image types
biological_titles = {
