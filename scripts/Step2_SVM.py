...
def svm(inp,prefix):    # Read data
    ...
    clf = SVC(kernel='rbf', probability=True, random_state=1234)
    param_grid = {'C': [0.01, 0.1, 1, 10, 100, 1000, 10000], 'gamma': ['scale', 'auto']}
    ...
    plt.savefig(f"{prefix}_svm_confusion_matrix.png")
    plt.close()
    ...
    plt.savefig(f"{prefix}_svm_roc_curve.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{prefix}_svm_metrics.png")
    plt.close()
