...
def xgboost(inp,prefix):
    ...
    clf = XGBClassifier(eval_metric='mlogloss', random_state=1234)
    param_grid = {'n_estimators': [10, 50, 100, 200, 300], 'max_depth': [3, 6, 9, 12, 15], 'learning_rate': [0.1, 0.01, 0.001, 0.0001]}
    ...
    plt.savefig(f"{prefix}_xgboost_confusion_matrix.png")
    plt.close()
    ...
    plt.savefig(f"{prefix}_xgboost_roc_curve.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{prefix}_xgboost_metrics.png")
    plt.close()
