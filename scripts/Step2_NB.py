...
def gaussian_nb_nested_cv(inp, prefix, use_elasticnet):
    # Read data
    ...
    for train_idx, test_idx in cv_outer.split(X, y_encoded):
        ...
        def objective_inner(trial):
            if use_elasticnet:
                ...
                # Create pipeline with StandardScaler, ElasticNet feature selection, and GaussianNB
                pipeline = Pipeline([
    ...
    if use_elasticnet:
        ...
        selected_features_df.to_csv(f"{prefix}_gaussiannb_selected_features.csv", index=False)
        print(f"Selected {selected_features_mask.sum()} features saved to {prefix}_gaussiannb_selected_features.csv")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run GaussianNB with Nested Cross-Validation, Optional ElasticNet Feature Selection, and Optuna hyperparameter optimization.')
    ...
    gaussian_nb_nested_cv(args.i, args.p, args.use_elasticnet)