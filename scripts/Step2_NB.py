...
def gaussian_nb(inp, prefix):
    ...
    def objective(trial):
        # GaussianNB does not have hyperparameters, so we just return cross-validation F1 score
        clf = GaussianNB()
    ...
    study.optimize(objective, n_trials=1, show_progress_bar=True)  # n_trials=1 since no hyperparameter tuning

    # Best model is just GaussianNB with no hyperparameters to tune
    ...
    joblib.dump((X_scaled, y_encoded, le), f"{prefix}_gaussiannb_data.pkl")

    # Output the best parameters
    print(f"Best model for GaussianNB: Default parameters")
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Run GaussianNB with Optuna hyperparameter optimization.')
    ...
    # Run the GaussianNB function
    gaussian_nb(args.input, args.output_prefix)