...
def logistic_regression_nested_cv(inp, prefix, use_elasticnet):
    # Read data
    ...
    for train_idx, test_idx in cv_outer.split(X, y_encoded):
        ...
        def objective_inner(trial):
            if use_elasticnet:
                ...
                # Create pipeline with StandardScaler, ElasticNet feature selection, and Logistic Regression
                pipeline = Pipeline([
                    ('scaler', StandardScaler()),
                    ('feature_selection', SelectFromModel(
                        ElasticNet(alpha=elasticnet_alpha, l1_ratio=l1_ratio, max_iter=10000, tol=1e-4, random_state=1234)
                    )),
                    ('logreg', LogisticRegression(C=C, random_state=1234, max_iter=1000))
                ])
            else:
                # Suggest hyperparameters for Logistic Regression
                C = trial.suggest_loguniform('C', 1e-2, 100)
    ...
    if use_elasticnet:
        ...
        best_model = Pipeline([
            ...
            ('logreg', LogisticRegression(C=best_C_full, random_state=1234, max_iter=1000))
        ])
    else:
        best_C_full = best_params_full['C']
    ...
    if use_elasticnet:
        ...
        selected_features_df.to_csv(f"{prefix}_logistic_regression_selected_features.csv", index=False)
        print(f"Selected {selected_features_mask.sum()} features saved to {prefix}_logistic_regression_selected_features.csv")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run Logistic Regression with Nested Cross-Validation, Optional ElasticNet Feature Selection, and Optuna hyperparameter optimization.')
    ...
    logistic_regression_nested_cv(args.i, args.p, args.use_elasticnet)