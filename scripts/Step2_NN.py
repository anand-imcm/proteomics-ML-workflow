...
def neural_network_nested_cv(inp, prefix, use_elasticnet):
    # Read data
    ...
    for train_idx, test_idx in cv_outer.split(X, y_encoded):
        ...
        def objective_inner(trial):
            if use_elasticnet:
                ...
                # Create pipeline with StandardScaler, ElasticNet feature selection, and MLPClassifier
                pipeline = Pipeline([
        ...
        if use_elasticnet:
            ...
            best_hidden_layer_size = best_params_inner['hidden_layer_size']
            best_hidden_layer_count = best_params_inner['hidden_layer_count']
            best_mlp_alpha = best_params_inner['mlp_alpha']
            best_learning_rate_init = best_params_inner['learning_rate_init']
            ...
            best_model_inner = Pipeline([
                ...
                ('mlp', MLPClassifier(hidden_layer_sizes=best_hidden_layers,
                                      activation='relu',
    ...
    if use_elasticnet:
        ...
        best_hidden_layer_size_full = best_params_full['hidden_layer_size']
        best_hidden_layer_count_full = best_params_full['hidden_layer_count']
        best_mlp_alpha_full = best_params_full['mlp_alpha']
        best_learning_rate_init_full = best_params_full['learning_rate_init']
        
        best_hidden_layers_full = tuple([best_hidden_layer_size_full] * best_hidden_layer_count_full)
        ...
        best_model = Pipeline([
            ...
            ('mlp', MLPClassifier(hidden_layer_sizes=best_hidden_layers_full,
                                  activation='relu',
                                  alpha=best_mlp_alpha_full,
                                  learning_rate_init=best_learning_rate_init_full,
                                  max_iter=200000,
                                  random_state=1234))
        ])
    else:
    ...
    if use_elasticnet:
        ...
        selected_features_df.to_csv(f"{prefix}_neural_network_selected_features.csv", index=False)
        print(f"Selected {selected_features_mask.sum()} features saved to {prefix}_neural_network_selected_features.csv")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run Neural Network model with Nested Cross-Validation, Optional ElasticNet Feature Selection, and Optuna hyperparameter optimization.')
    ...
    neural_network_nested_cv(args.i, args.p, args.use_elasticnet)