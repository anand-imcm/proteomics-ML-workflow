...
map2String <- function(string_db, Pro_Plot_F, Full_SHAP_F_Plot, score_thresholdHere, combined_score_thresholdHere, CoPro_EntrezSym, patternChosen, png_cont){
  ...
  if (is.null(edges) || nrow(edges) == 0) {
    ...
    image.plot(legend.only = TRUE, 
               ...
               legend.shrink = 0.3, 
               legend.mar = 3,
               legend.args = list(text = "Standardised SHAP Value", side = 1, font = 2, line = 1, cex = 0.8),
    ...
    legend(x = "bottom",                   
           inset = c(0, -0.09),                
           legend = c("High SHAP Protein", "Coexpressed Interacter"),
           pch = c(21, 22),            
           pt.bg = c("white", "white"),
           pt.cex = 1.2,
           cex = 0.7,
...
### Standardize the SHAP magnitude per model to -1~1, meanwhile 0 as 0. Keep Full_SHAP_F for further reference
Full_SHAP_F_AllScaled <- as.data.frame(lapply(Full_SHAP_F, custom_signed_scaler))
rownames(Full_SHAP_F_AllScaled) <- rownames(Full_SHAP_F)
colnames(Full_SHAP_F_AllScaled) <- colnames(Full_SHAP_F) ### This column name assignment is necessary because dashes (-) in the original names may have been automatically converted to periods (.) 
...
for(colCt in colnames(Full_SHAP_F_AllScaled)){
  ...
  tryCatch(
    {if(all(Full_SHAP_F_AllScaled[,colCt] == 0)){message(paste0("For ", colCt, " All proteins have identical importance scores, so the most important ones cannot be distinguished."))
      ...
      SHAP_PlotF <- Full_SHAP_F_AllScaled %>% dplyr::select(all_of(colCt)) %>% dplyr::filter(!!sym(colCt) != 0) %>% dplyr::arrange(desc(abs(!!sym(colCt)))) ### Exclude proteins with SHAP=0 for PPI analysis
